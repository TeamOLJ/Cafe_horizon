...
class CouponHistoryFragment : Fragment() {
        private var _binding: FragmentCouponHistoryBinding? = null
        ...
        override fun onCreateView(
            ...
            _binding = FragmentCouponHistoryBinding.inflate(inflater, container, false)
            val view = binding.root

            val data: MutableList<Coupons> = loadCoupons()

            var adapter = HistoryRecyclerAdapter()
            adapter.datas = data
            binding.recyclerViewCHistory.adapter = adapter
            binding.recyclerViewCHistory.layoutManager = LinearLayoutManager(this.context)
